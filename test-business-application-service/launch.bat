...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "test-business-application-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name test-business-application-service apps/test-business-application-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app test-business-application-service:1.0-SNAPSHOT
		call oc expose svc/test-business-application-service
	) else (
		echo "Launching the application locally..."
		cd test-business-application-service
		cd target
