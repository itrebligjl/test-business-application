...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "test-business-application-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name test-business-application-service apps/test-business-application-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app test-business-application-service:1.0-SNAPSHOT
    oc expose svc/test-business-application-service
else

	echo "Launching the application locally..."
	pattern="test-business-application-service"
	files=( $pattern )
	cd ${files[0]}
